...
for category in $(seq 1 "$categories"); do
  ...
  echo -e \
    "Some of the name translations of category \"$current_id\" have non-unique lang attributes. Check for occurrences of the following duplicated lang attribute(s) in the name elements of category \"$current_id\": "$(comm -2 -3 <(echo -e "$langcodes") <(echo -e "$uniquelangcodes") | tr '\n' ' ')".\n---"
done
